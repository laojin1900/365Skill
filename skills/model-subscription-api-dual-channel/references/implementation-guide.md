...
### 8.4 超长列表的厂商二次折叠（vendor secondary-fold，渐进式三级）

适用：某个 provider 有 **≥ 24 个模型**（如 `openrouter` 271 个、`B-ai` 39 个）时，简单的“前 4/N”折叠不够——展开后一次几百行。**不要直接弹厂商视图**，而是做渐进式三级展开：

1. **折叠态（默认）**：直接展示**最常用的前 4 个模型**（与小型 provider 的 `前 4/N` 折叠完全一致，看不到任何厂商分组），按钮为 `▼ 展开更多 (还有 N 个模型)`。
2. **点“展开更多”** → 切换到厂商分组视图：provider 头部变为 `N 个 · M 厂商`；每个厂商渲染成**折叠的头部行 `▶ vendor N 个`**（只有名字+数量，**不显示任何模型**）；只显示前 6 个厂商，其余藏在 `▼ 展开更多厂商 (还有 N 个)` 后面。
3. **点某个厂商头部** → **此时才**展开该厂商的模型（`▶` 旋转 90°，该厂商全部模型渲染出来）；再点一次收起。

视图底部有 `▲ 折叠收起 (回到常用 4 个)` 按钮，一键回到第 1 层。
...
- 阈值以下的 provider 保持原有“前 4/N”折叠——它的展开显示**全部模型平铺**、不做厂商分组；搜索时两级折叠都跳过。
- 状态分工：`expandedProviders[provider]` 管第 2 层 vs 第 1 层，`expandedVendors['provider/vendor']` 管第 3 层。

### 8.5 迭代演进记录（这个折叠方案是怎么改出来的）

> 沉淀自 Pi Web 真实迭代过程。别一上来就做三级——先做简单版，再按用户反馈逐层收敛。

**v1（初版）——厂商分组直接弹出**：超长 provider（≥24 个模型）默认就显示厂商分组：每个厂商头部下直接带 3 个模型预览 + 每厂商一个“展开 N 个”按钮，头部写 `N 个 · M 厂商`。

**用户反馈 1**：不要一打开就看到厂商分组。*“像 OpenRouter 这种，你默认展示 4 个模型就可以了——4 个直接的最常用模型。点击更多才展示按厂商的模型列表。点击某个厂商的列表，才展示他的常用模型。”*

→ **v2（现版）——渐进式三级**：

1. **折叠态 = 前 4 个最常用模型**（与小型 provider 完全一致的 `前 4/N`，无任何厂商痕迹）；
2. **点“展开更多”** → 厂商分组视图：头部 `N 个 · M 厂商`，每个厂商是**折叠的头部行 `▶ vendor N 个`**（只显示名字+数量，模型全隐藏），只渲染前 6 个厂商 + `▼ 展开更多厂商`；
3. **点某个厂商头部** → 此时才展开该厂商的模型（▶ 旋转 90°），再点收起；

底部加 `▲ 折叠收起 (回到常用 4 个)` 一键回到第 1 层。

**这次迭代沉淀出的 UX 原则**：

- **信息密度按“使用频率”递减**：用户每天切换的是少数几个常用模型——默认视图永远是“最常用的 4 个”，而不是结构视图。分组/厂商/完整列表只是**按需深入的路径**。
- **每次“展开”只揭一层**：折叠 → 常用 4 个；更多 → 厂商列表；点厂商 → 该厂商模型。任何一层都不应一次性倾倒全部内容（271 行直接展开 = 失败）。
- **厂商头部本身就是开关**：不要“头部 + 每厂商展开按钮”两个控件——一个可点的头部行（▶/▼ 指示）就够了，控件越少越好。
- **每层都要能回退**：厂商视图底部必须有“折叠收起（回到常用 4 个）”，不能把用户困在深层视图。
- **删除死代码**：v1 引入的每厂商 3 行预览 + “展开 N 个”按钮 + `MODEL_VENDOR_SHOW_LIMIT` 常量在 v2 全部删除（厂商头即开关）。迭代中及时清理不再使用的常量/控件。

**验证方式（Playwright 断言）**：

- 折叠态：`前 4/39` 出现、`▶vendor N 个` 不出现；
- 展开后：`39 个 · 8 厂商` 出现、厂商列表为 `▶deepseek2 个 / ▶kimi3 个 / ▶gemini5 个 / ▶claude10 个 / ▶glm3 个 / ▶gpt12 个`、模型行计数 = 0；
- 点 `▶gemini` 后：gemini 模型行 = 5；
- 点“折叠收起”后：回到 `前 4/39`、厂商视图消失。
