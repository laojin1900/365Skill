...
用户明确回复"继续"（或等价表述）后，**本会话内**不再重复此提醒（会话里第一个 B 级任务仍停等一次）。用户切换模型后重发需求的，按新模型正常执行。
...
- 「如何验证」**先列本机验不了的部分**（密钥、环境、权限、公网回调）及其替代（生产验 / 用户亲点 / 夹具渲染）——记忆里已有的环境限制此时对照，别等实施完才发现验证对象不可达
...
- C 级：派更高强度的独立视角做终审
- 检查清单：需求符合性 / 范围膨胀 / 无关文件改动 / 重复建设 / 权限与数据风险 / 错误处理 / 测试真实存在 / **写进外部系统或客户可读表的每个字符串谁能看到**（买家 / 商户 / 运维——供应商身份与内部依据不得进买家面）
